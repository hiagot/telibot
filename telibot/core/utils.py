...
def post_guild(guild: discord.Guild) -> dict:
    ...
    resp = requests.post(f"http://{settings.API_URL}/guilds/", json=payload, timeout=2)
...
def post_user(user: discord.Member) -> dict:
    ...
    resp = requests.post(f"http://{settings.API_URL}/users/", json=payload, timeout=2)
    return {"status_code": resp.status_code, "response": resp.json()}
...
def post_message(message: discord.Message) -> dict:
    ...
    resp = requests.post(f"http://{settings.API_URL}/messages/", json=payload, timeout=2)
    return {"status_code": resp.status_code, "response": resp.json()}
